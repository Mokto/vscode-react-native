...
import {Adb, AndroidAPILevel, IDevice, DeviceType} from "../../common/android/adb";
...
export class AndroidPlatform implements IAppPlatform {
    ...
    public runApp(runOptions: IRunOptions, shouldLaunchInAllDevices: boolean = false): Q.Promise<void> {
            ...
            return output
                .finally(() => {
                    ...
                    if (reason.message === AndroidPlatform.MULTIPLE_DEVICES_ERROR && this.devices.length > 1 && this.debugTarget) {
                        ...
                        const devices = shouldLaunchInAllDevices
                            ? this.adb.getOnlineDevices()
                            : Q([this.debugTarget]);
                        return new PromiseUtil().forEach(devices, device => {
                            return this.launchAppWithADBReverseAndLogCat(device, runOptions);
                        });
                    } else {
    ...
    private launchAppWithADBReverseAndLogCat(device: IDevice, runOptions: IRunOptions): Q.Promise<void> {
        return Q({})
            .then(() => {
                return this.adb.launchApp(runOptions.projectRoot, this.packageName, device.id);
            }).then(() => {
                return this.configureADBReverseWhenApplicable(device);
            }).then(() => {
                return this.startMonitoringLogCat(device, runOptions.logCatArguments).catch(error => // The LogCatMonitor failing won't stop the debugging experience
                    Log.logWarning("Couldn't start LogCat monitor", error));
            });
    }

    public enableJSDebuggingMode(runOptions: IRunOptions): Q.Promise<void> {
        return this.adb.reloadAppInDebugMode(runOptions.projectRoot, this.packageName, this.debugTarget.id);
    }
            ...
            return Q({}) // For other emulators and devices we try to enable adb reverse
                ...
                .then(apiVersion => {
                    if (apiVersion >= AndroidAPILevel.LOLLIPOP) { // If we support adb reverse
                        return this.adb.reverseAdd(device.id, Packager.PORT, Packager.PORT);
                    } else {
     ...
     */
    private getTargetEmulator(runOptions: IRunOptions, devices: IDevice[]): IDevice {
    ...
    private startMonitoringLogCat(device: IDevice, logCatArguments: string): Q.Promise<void> {
        return this.extensionMessageSender.sendMessage(ExtensionMessage.START_MONITORING_LOGCAT, [device.id, logCatArguments]);
    }
}